...
from src.schemas import TagModel, TagResponse, User
...
async def read_tag(tag_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    tag_service = TagService(db)
...
async def create_tag(body: TagModel, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    tag_service = TagService(db)
...
async def remove_tag(tag_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    tag_service = TagService(db)
